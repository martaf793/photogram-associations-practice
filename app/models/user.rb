...
class User < ApplicationRecord
  ...
  has_many(:sent_follow_request, class_name:"FollowRequest", foreign_key: "sender_id") 
  ...
  has_many(:received_follow_request, class_name:"FollowRequest", foreign_key: "recipient_id")
  ...
  # User#accepted_sent_follow_requests: returns rows from the follow requests table associated to this user by the sender_id column, where status is 'accepted'
    ...
    scope (:accepted_sent_follow_requests, -> {where(status: "accepted")}, class_name: "FollowRequest", foreign_key: :sender_id) #status: "accepted"
  ...
  scope (:accepted_received_follow_requests, -> {where(status: "accepted")}, class_name: "FollowRequest", foreign_key: :recipient_id)
...
has_many(:commented_photos, through: :comments, source: :photo)
  ...
  has_many(:followers, through: :accepted_received_follow_requests, source: :sender) #sender, not user 
  ...
  has_many(:feed, through: :leaders, source: :own_photos) #own_photos, no photo

  # scope :leaders_, -> {where(sender_id: true)}
